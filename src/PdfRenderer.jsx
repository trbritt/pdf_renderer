...
function PdfRenderer({ pdfUrl }) {
    ...
    return (
        <>
            <motion.div
                className={`progress-bar z-20 shadow-[0_10px_15px_-1px_rgba(0,0,0,0.1)] ${current_pbar_width === 100 ? 'shadow-red-500' : null}`}
                style={{ width: `${current_pbar_width}%`, stiffness: 100, damping: 30, restDelta: 0.001 }}
            ...
            <div  initial="hidden" animate="show" className='flex py-20 justify-evenly backdrop-blur-3xl' ref={containerRef} id='pdf-viewer' onWheel={handleWheel}>
                ...
                >
                    {Array.from(new Array(numPages), (el, index) => (
                        <div className={`${index + 1 === currentPage ? 'flex' : 'hidden'} gap-20`}>
                            <div animate={index + 1 <= numPages ? show : hide}>
                                {index + 1 <= numPages ? <Page id='pagechild' key={index + 1} pageNumber={index + 1}  onLoadSuccess={handlePageLoadSuccess} height={document.getElementById('app')?.clientHeight*0.78 ?? 1000} /> : null}
                            </div>
                            <div animate={index + 1 <= numPages ? show : hide} className='xs:max-lg:hidden'>
                                {index + 2 <= numPages ? <Page key={index + 2} pageNumber={index + 2} onLoadSuccess={handlePageLoadSuccess} height={document.getElementById('app')?.clientHeight*0.78 ?? 1000} /> : null}
                            </div>
                        </div>
                    ))}
                </Document>
